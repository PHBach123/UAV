...
void Calc(double q1, double q2, double d) {
	...
	double fd = Vmax * 5 * pow(10,9) / (3 * pow(10,8));
	double z = _j0(2 * M_PI * fd * 0.0001) * _j0(2 * M_PI * fd * 0.0001);
	
	//printf("%lf",z);
	//printf("%lf",fd);
	double sigmau = Noise * (1 + (1 - z * z));
	
	double sigmad = Noise * (1 + (1 - z * z) * (1 - z * z) + (1 - z * z));
	
	f22a = B *DelT*Tn / log(2) * log(1 + (exp(-E) * w0 / sigmad) * (Nmax * w0 * Ps * pow(z,4) + Pu * ceil(sqrt(Noise)) * pow(dsu,alpha) * pow(z,2)) / pow(dsu,alpha) / pow(ddu,alpha) );
	
	f22b = B *DelT* Tn / log(2) * log(1 + (exp(-E) * w0 * Ps * pow(z,2)) / pow(dsu,alpha) / sigmau);
